...
export type KeyHook = (event: KeyboardEvent) => boolean;

interface KeymapManagerOptions {
  platform: string;
...
export class KeymapManager {
  ...
  constructor({ platform, commandManager }: KeymapManagerOptions) {
  ...
  getAllFormattedKeyByCommand() {
    const keymap = this.getKeymap();
    const formattedKeyByCommand: Record<string, string> = {};
    for (const key in keymap) {
      formattedKeyByCommand[keymap[key]] = this.formatKeyDescriptor(key);
    }
    return formattedKeyByCommand;
  }

  getFormattedKey(command: string) {
    const keys = this.getAllFormattedKeyByCommand();
    return keys[command];
  }
  ...
  normalizeKeyDescriptor(descriptor: string) {
    ...
    key = keyDescriptor[0];
    if (key === "delete" && this.platform === "darwin") {
      key = "backspace";
    }
  ...
  translateKeyboardEvent(event: KeyboardEvent) {
    ...
    if (ident && (ident.codePointAt(0) || 0) <= 0x007f) {
      key = ident;
    }
  ...
  inEditMode(): boolean {
    return (
      (document.activeElement?.nodeName === "TEXTAREA" ||
        ...
        document.activeElement?.classList.contains("cm-content") || // codemirror editor
        document.activeElement?.classList.contains("ProseMirror")) ?? // promise mirror
      false
    );
  }
    ...
    window.document.body.addEventListener(
      ...
      (event) => {
        ...
        if (this.inEditMode() || this.inModalDialog()) {
          ...
          const handled = this.handleKey(this.translateKeyboardEvent(event));
          if (handled) {
