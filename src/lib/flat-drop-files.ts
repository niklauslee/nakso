// code from: https://github.com/placemark/flat-drop-files
...
function getHandle(
  ...
  return (item as any).getAsFileSystemHandle().catch((e: any): any => {
    console.error(e);
    return null;
...
function readFile(
  ...
  return Promise.all([getFile(entry), getHandle(item)]).then(
    ([file, handle]) => {
      ...
      // Commented out because it causes errors in electron.
