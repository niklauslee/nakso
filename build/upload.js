/**
 * Upload build artifacts to remote S3 storage.
 *
...
async function ensureLatestJson() {
  ...
  try {
    ...
    if (!latestJson) {
      ...
      console.log("[upload] a new latest.json has been created and uploaded.");
...
function checkBundleArtifacts(rustTarget) {
  ...
  if (!fs.existsSync(installerPath)) {
    console.error(`[upload] error: installer not found at ${installerPath}`);
    return false;
  }
  if (hasBundle && !fs.existsSync(bundlePath)) {
    console.error(`[upload] error: bundle not found at ${bundlePath}`);
    return false;
  }
  if (!fs.existsSync(signaturePath)) {
    console.error(`[upload] error: signature not found at ${signaturePath}`);
    return false;
  }
...
async function uploadBundleArtifacts(rustTarget, platform, arch, version) {
  ...
  try {
    ...
    console.log(`[upload] uploaded installer: ${installerKey}`);
...
async function main() {
  ...
  console.log(
    `[upload] uploading artifacts - ${target} (${rustTarget}) ${currentVersion}`
  );
  ...
  try {
    ...
    // if try to upload older version, warn and exit
    if (semver.lt(currentVersion, latestJson.version)) {
      console.warn(
        `[upload] warning: package.json version (${currentVersion}) is older than latest.json version (${latestJson.version})`
      );
      console.warn(
        "[upload] please ensure you are uploading the correct version."
      );
      return;
    }

    // if try to upload newer version, create new latest.json
    if (semver.gt(currentVersion, latestJson.version)) {
      console.log(`[upload] newer version detected: ${currentVersion}`);
      const newLatestJson = generateNewLatestJson();
      await uploadLatestJson(newLatestJson);
      latestJson = newLatestJson;
      console.log("[upload] latest.json has been updated.");
    }

    // if versions are equal, upload the platform-specific bundle
    if (semver.eq(currentVersion, latestJson.version)) {
      // check if platform is already uploaded
      if (latestJson.platforms[target]) {
        console.warn(
          `[upload] warning: platform ${target} is already uploaded in latest.json.`
        );
        console.warn("[upload] skipping upload for this platform.");
        return;
      }
      ...
      console.log("[upload] latest.json has been updated with new platform.");
      console.log(`[upload] platform ${target} successfully uploaded.`);
    }
  } catch (error) {
